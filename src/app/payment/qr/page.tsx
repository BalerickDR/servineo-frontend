...
export default function PaymentsPage() {
  ...
  //jhoel
  const type = searchParams.get('type') || 'service'; // "service" o "wallet"
  // Datos condicionales según tipo
  const fixerId = searchParams.get('fixerId') || null;

  const isWallet = type === 'wallet';
  const titulo = isWallet ? 'Recarga de Saldo' : 'Pagos con QR';
  const subTitulo = isWallet
    ? 'Escanea el código QR para recargar tu wallet'
    : 'Información de pago';
    ...
    (async () => {
      try {
        ...
        const payload = isWallet
          ? { fixerId, amount, currency, type: 'wallet' } // 👈 recarga de saldo
          : { bookingId, providerId, amount, currency }; // 👈 pago normal
      ...
      <header className="bg-[#2B6AE0]">
        <div className="max-w-5xl px-6 py-6">
          <h1 className="text-5xl font-semibold text-white">
            {isWallet ? 'Recarga de Saldo (QR)' : 'Pagos con QR'}
          </h1>
        </div>
      </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
          <section className="md:-ml-35">
            <h2 className="text-4xl font-semibold mb-3 text-black">
              {isWallet ? 'Recarga de Saldo' : 'Información de pago'}
            </h2>
            ...
            <dl className="space-y-6 text-black">
              ...
              {/* --- Fila: Número de Trabajo (solo para servicio) --- */}
              {!isWallet && (
              ...
              {/* --- Si NO es wallet, mostrar Subtotal y Comisión --- */}
              {!isWallet && (
                <>
                  <div className="relative min-h-8">
                    <dt className="text-2xl font-medium inline-block w-[160px] text-left">
              ...
              {/* --- Fila: Total --- */}
              <div className="relative min-h-8">
                <dt className="text-2xl font-semibold inline-block w-[160px] text-left">Total:</dt>
              ...
              <div className="relative min-h-8 mt-6">
                <dt className="text-2xl font-medium inline-block w-[160px] text-left">Estado:</dt>
          ...
          <aside className="bg-[#759AE0] rounded-xl p-5 md:justify-self-end w-full md:w-[420px] md:ml-16 md:self-center">
            <h3 className="text-2xl font-semibold text-gray-900 mb-4 text-center">
              {isWallet ? 'Escanea para recargar tu saldo' : 'Escanea el código QR'}
            </h3>
