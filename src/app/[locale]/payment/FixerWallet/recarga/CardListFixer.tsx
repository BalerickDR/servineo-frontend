...
export default function CardListFixer({
 ...
 try {
 const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/cards?userId=${fixerId}`);
...
            const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/wallet/update`, {
                method: 'POST',
...
            });
      ...
      showSuccessModal(
        `💰 Recarga exitosa: +${amount} BOB (nuevo balance: ${data.wallet.balance} BOB)`,
      );
      onRechargeSuccess?.();
    } catch (err) {
      console.error(err);
      alert(
        'Error al recargar la wallet. Asegúrate de que tu monto sea válido y la tarjeta tenga fondos.',
      );
    } finally {
